...
func RunCommandHandler(bot *tgbotapi.BotAPI, filteredChatID string, client *system_works.Client) {
	if bot == nil {
		system_works.LogWarn("Bot is nil, command handler not started")
		return
	}

	if filteredChatID == "" {
		system_works.LogWarn("Filtered chat ID is empty, command handler not started")
		return
	}

	system_works.LogInfo("Starting command handler", zap.String("filteredChatID", filteredChatID))
	...
	for update := range updates {
		...
		if update.Message.IsCommand() {
			...
			system_works.LogDebug("Received command",
...
func handleHelpCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message) {
	...
	// + help in caption
	photo := tgbotapi.NewPhoto(message.Chat.ID, tgbotapi.FilePath("asty1.jpeg"))
	photo.Caption = helpText
	photo.ParseMode = tgbotapi.ModeHTML
	photo.ReplyToMessageID = message.MessageID
	_, err := bot.Send(photo)
	if err != nil {
		system_works.LogError("Failed to send /helps photo message", zap.Error(err))
		return
	}

	system_works.LogInfo("Help message sent",
...
func handleAddTokenCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message, ticker string) {
	...
	poolLpPublicKey, err := system_works.FindPoolLpPublicKeyByTicker(ticker)
	if err != nil {
		system_works.LogWarn("Failed to find token by ticker",
		...
		_, err := bot.Send(msg)
		if err != nil {
			system_works.LogError("Failed to send error message", zap.Error(err))
		}
		return
	}

	// Check, token in
	existingTokens, err := system_works.LoadFilteredTokens()
	if err == nil {
		for _, existingToken := range existingTokens {
			if strings.TrimSpace(existingToken) == poolLpPublicKey {
				...
				system_works.LogDebug("Token already in filtered list",
					zap.String("ticker", ticker),
	...
	err = system_works.AddFilteredToken(poolLpPublicKey)
	if err != nil {
		system_works.LogError("Failed to add filtered token",
			zap.String("ticker", ticker),
			zap.String("poolLpPublicKey", poolLpPublicKey),
			zap.Error(err))
	...
	// Check, token
	verifyTokens, err := system_works.LoadFilteredTokens()
	if err == nil {
		found := false
		...
		if !found {
			system_works.LogError("Token was not saved to file after AddFilteredToken",
				zap.String("ticker", ticker),
				zap.String("poolLpPublicKey", poolLpPublicKey))
			msg := tgbotapi.NewMessage(message.Chat.ID,
				"An error occurred, please try again later")
	...
	_, err = bot.Send(msg)
	if err != nil {
		system_works.LogError("Failed to send success message", zap.Error(err))
	}

	system_works.LogInfo("Token added to filtered list via command",
		zap.String("ticker", ticker),
		zap.String("poolLpPublicKey", poolLpPublicKey),
		zap.String("chatID", formatChatID(message.Chat.ID)),
		zap.String("username", message.From.UserName))
...
func handleDeleteTokenCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message, ticker string) {
	...
	err = system_works.RemoveFilteredToken(poolLpPublicKey)
	if err != nil {
		if err.Error() == "token not found in list" {
			...
			system_works.LogDebug("Token not found in filtered list",
				zap.String("ticker", ticker),
				zap.String("poolLpPublicKey", poolLpPublicKey))
		} else {
			system_works.LogError("Failed to remove filtered token",
				zap.String("ticker", ticker),
				zap.String("poolLpPublicKey", poolLpPublicKey),
				zap.Error(err))
	...
	system_works.LogInfo("Token removed from filtered list via command",
		zap.String("ticker", ticker),
		zap.String("poolLpPublicKey", poolLpPublicKey),
		zap.String("chatID", formatChatID(message.Chat.ID)),
		zap.String("username", message.From.UserName))
...
func handleFlashReportCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message, ticker string, dateStr string, client *system_works.Client) {
	// Generate
	report, err := system_works.GenerateHoldersReport(ticker, dateStr, client)
	if err != nil {
		system_works.LogError("Failed to generate holders report",
			zap.String("ticker", ticker),
	...
	_, err = bot.Send(msg)
	if err != nil {
		system_works.LogError("Failed to send report message", zap.Error(err))
	}

	system_works.LogInfo("Holders report generated and sent",
		zap.String("ticker", ticker),
		zap.String("dateStr", dateStr),
		zap.String("chatID", formatChatID(message.Chat.ID)),
		zap.String("username", message.From.UserName))
...
func handleFlowReportCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message, ticker string, dateStr string) {
	// Generate
	report, err := system_works.GenerateFlowReport(ticker, dateStr)
	if err != nil {
		system_works.LogError("Failed to generate flow report",
			zap.String("ticker", ticker),
			zap.String("dateStr", dateStr),
			zap.Error(err))
	...
	_, err = bot.Send(msg)
	if err != nil {
		system_works.LogError("Failed to send flow report", zap.Error(err))
		return
	}

	system_works.LogInfo("Flow report sent via command",
		zap.String("ticker", ticker),
		zap.String("dateStr", dateStr),
		zap.String("chatID", formatChatID(message.Chat.ID)),
		zap.String("username", message.From.UserName))
...
func handleStatsCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message) {
	...
	if err != nil {
		system_works.LogError("Failed to get stats",
			zap.Error(err))
	...
	if err := system_works.SaveStatsData(stats, checkFlag); err != nil {
		system_works.LogWarn("Failed to save stats data", zap.Error(err))
	}
	...
	if err != nil {
		...
		system_works.LogInfo("Sending stats chart", zap.String("chartPath", chartPath))
		...
		_, err = bot.Send(photo)
		if err != nil {
			system_works.LogError("Failed to send stats chart", zap.String("chartPath", chartPath), zap.Error(err))
			msg := tgbotapi.NewMessage(message.Chat.ID, statsMessage)
			msg.ParseMode = tgbotapi.ModeHTML
			msg.ReplyMarkup = keyboard
			bot.Send(msg)
	...
	system_works.LogInfo("Stats sent via command",
		zap.String("chatID", formatChatID(message.Chat.ID)),
		zap.String("username", message.From.UserName))
}
...
func handleSparkCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message) {
	...
	if err != nil {
		system_works.LogError("Failed to get BTC spark reserve",
			zap.Error(err))
	...
	if err := system_works.SaveBTCSparkData(btcReserve, checkFlag); err != nil {
		system_works.LogWarn("Failed to save BTC spark data", zap.Error(err))
	}
	...
	if err != nil {
		...
		_, err = bot.Send(msg)
		if err != nil {
			system_works.LogError("Failed to send spark message", zap.Error(err))
			return
		}
		return
	}
	...
	if fileInfo.Size() == 0 {
		system_works.LogError("Chart file is empty", zap.String("chartPath", chartPath))
		keyboard := tgbotapi.NewInlineKeyboardMarkup(
			tgbotapi.NewInlineKeyboardRow(
				tgbotapi.NewInlineKeyboardButtonURL("Trade on Luminex", "https://luminex.io/"),
			),
	...
	system_works.LogInfo("Sending BTC spark chart",
	...
	_, err = bot.Send(photo)
	if err != nil {
		system_works.LogError("Failed to send BTC spark chart", zap.String("chartPath", chartPath), zap.Error(err))
		msg := tgbotapi.NewMessage(message.Chat.ID, sparkMessage)
		msg.ParseMode = tgbotapi.ModeHTML
		msg.ReplyMarkup = keyboard
		bot.Send(msg)
	...
	system_works.LogInfo("BTC spark sent via command",
		zap.String("chatID", formatChatID(message.Chat.ID)),
		zap.String("username", message.From.UserName),
...
func formatStatsMessage(stats *system_works.StatsResponse) (string, error) {
	// Get in moscow time
	moscowLocation, _ := time.LoadLocation("Europe/Moscow")
	currentTime := time.Now().In(moscowLocation)
	dateStr := currentTime.Format("02 Jan")
	...
	tvlFormatted := system_works.FormatUSDValue(stats.TotalTVLUSD)
	volumeFormatted := system_works.FormatUSDValue(stats.TotalVolume24HUSD)

	message := fmt.Sprintf("Stats on %s:\n\n", dateStr)

	// Get -5 tokens - if error, return error
	topTokens, err := system_works.GetTopTokens(5)
	if err != nil {
	...
	if len(topTokens) > 0 {
		...
		for i, token := range topTokens {
			marketCapFormatted := system_works.FormatUSDValue(token.MarketCapUSD)
			volumeFormatted := system_works.FormatUSDValue(token.Volume24HUSD)
	...
	// If save
	if len(message) > 1024 {
		system_works.LogWarn("Stats message too long, reducing spaces",
