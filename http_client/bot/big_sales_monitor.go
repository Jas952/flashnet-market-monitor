...
func formatSwapMessageBig(swap system_works.Swap) string {
	swapType := swap.GetSwapType()

	var typeEmoji, typeLabel string
	switch swapType {
	case system_works.SwapTypeBuy:
		typeEmoji = "🟢"
		typeLabel = "ПОКУПКА"
	case system_works.SwapTypeSell:
		typeEmoji = "🔴"
		typeLabel = "ПРОДАЖА"
	default:
		typeEmoji = "🔄"
	...
	if swapType == system_works.SwapTypeBuy {
	...
	} else if swapType == system_works.SwapTypeSell {
...
func getBTCAmountFromSwap(swap system_works.Swap) float64 {
	swapType := swap.GetSwapType()
	...
	if swapType == system_works.SwapTypeBuy {
		...
		system_works.LogDebug("Swap is not buy/sell, returning 0 BTC value",
	...
	if satoshiStr == "" {
		system_works.LogWarn("Empty AmountIn/AmountOut for buy/sell swap, returning 0 BTC value",
			zap.String("swapType", string(swapType)),
	...
	if err != nil || n != 1 {
		system_works.LogWarn("Failed to parse AmountIn/AmountOut, returning 0 BTC value",
			zap.String("swapType", string(swapType)),
	...
	system_works.LogDebug("Calculated BTC value from swap",
		zap.String("swapType", string(swapType)),
		zap.String("satoshiStr", satoshiStr),
...
func shouldSendSwap(swap system_works.Swap, minBTCAmount float64) bool {
...
func getTokenAmountFromSwap(swap system_works.Swap, amountStr string, tokenMetadata *system_works.TokenMetadata) string {
	if amountStr == "" {
		return ""
	}
	...
	if tokenMetadata != nil && tokenMetadata.Ticker != "" {
		decimals = system_works.GetTokenDecimals(swap.PoolLpPublicKey, swap, tokenMetadata.Ticker)
	}
...
func saveHolderFromSwap(swap system_works.Swap) {
	ticker, err := system_works.GetTickerFromPoolLpPublicKey(swap.PoolLpPublicKey)
	if err != nil {
		system_works.LogDebug("Failed to get ticker from poolLpPublicKey", zap.String("poolLpPublicKey", swap.PoolLpPublicKey), zap.Error(err))
		return
	}

	if ticker == "" {
		system_works.LogDebug("Ticker is empty, skipping holder save", zap.String("poolLpPublicKey", swap.PoolLpPublicKey))
		return
	}

	// Check, ticker for ASTY, SOON, BITTY)
	if !system_works.IsTickerAllowed(ticker) {
		system_works.LogDebug("Ticker not in allowed list, skipping holder save", zap.String("ticker", ticker))
		return
	}

	// Get balance token API
	// API: https://api.luminex.io/spark/address/{swapperPublicKey}
	_, currentAmount, err := system_works.GetTokenBalanceFromWallet(swap.SwapperPublicKey, ticker)
	if err != nil {
		system_works.LogDebug("Failed to get current token balance from API", zap.String("address", swap.SwapperPublicKey), zap.String("ticker", ticker), zap.Error(err))
		return
	}

	savedData, err := system_works.LoadSavedHolders(ticker)
	if err != nil {
		system_works.LogWarn("Failed to load saved holders", zap.String("ticker", ticker), zap.Error(err))
		return
	}
	...
	if !exists {
		...
		if currentAmount == 0 {
			...
			system_works.LogDebug("New address has balance below threshold, skipping", zap.String("ticker", ticker), zap.String("address", swap.SwapperPublicKey), zap.Float64("amount", currentAmount))
			return
		} else {
			// on swap'
			if swapType == system_works.SwapTypeBuy {
		...
		if currentAmount == 0 {
			...
			// balance -
			system_works.LogDebug("Balance unchanged, skipping update", zap.String("ticker", ticker), zap.String("address", swap.SwapperPublicKey), zap.Float64("amount", currentAmount))
			return
		}
	}
	...
	if err := system_works.SaveSavedHolders(ticker, savedData); err != nil {
		system_works.LogWarn("Failed to save saved holders", zap.String("ticker", ticker), zap.Error(err))
		return
	}
	...
	if err := system_works.UpdateDynamicHoldersFromSwap(ticker, swap.SwapperPublicKey, currentAmount, previousAmount, action, btcValue); err != nil {
		system_works.LogWarn("Failed to update dynamic holders", zap.String("ticker", ticker), zap.String("address", swap.SwapperPublicKey), zap.Error(err))
		return
	}

	// Update flow data for invested and sold, for liquidated)
	if action == "invested" || action == "sold" {
		if err := system_works.UpdateFlowFromSwap(ticker, action, btcValue); err != nil {
			system_works.LogWarn("Failed to update flow from swap", zap.String("ticker", ticker), zap.String("action", action), zap.Error(err))
		}
	}

	system_works.LogDebug("Updated holder from swap",
		zap.String("ticker", ticker),
...
// formatSwapMessageForTelegram for in Telegram
func formatSwapMessageForTelegram(client *system_works.Client, swap system_works.Swap) (string, string) {
	swapType := swap.GetSwapType()
	btcAmount := getBTCAmountFromSwap(swap)
	btcAmountStr := formatBTCWithoutTrailingZeros(btcAmount)

	// Get token from Luminex API
	tokenMetadata := system_works.GetTokenMetadata(swap.PoolLpPublicKey)

	// Get from Luminex API for
	marketcap := system_works.GetPoolMarketCap(swap.PoolLpPublicKey, swap)
	...
	balanceResp, err := system_works.GetWalletBalance(swap.SwapperPublicKey)

	// Get (username) for "wallet"
	username := system_works.GetWalletUsername(swap.SwapperPublicKey)
	...
	firstBuyDate := ""
	if client != nil {
		firstBuyDateStr, err := system_works.GetFirstBuySwap(client, swap.SwapperPublicKey, swap.PoolLpPublicKey)
		if err != nil {
			system_works.LogDebug("Failed to get first buy swap",
				zap.String("swapperPublicKey", swap.SwapperPublicKey),
				zap.String("poolLpPublicKey", swap.PoolLpPublicKey),
				zap.Error(err))
		} else if firstBuyDateStr != "" {
			firstBuyDate = fmt.Sprintf("First buy - %s\n", firstBuyDateStr)
			system_works.LogDebug("First buy date added to message",
				zap.String("swapperPublicKey", swap.SwapperPublicKey),
				zap.String("poolLpPublicKey", swap.PoolLpPublicKey),
				zap.String("firstBuyDate", firstBuyDateStr))
		} else {
			system_works.LogDebug("First buy date is empty",
				zap.String("swapperPublicKey", swap.SwapperPublicKey),
				zap.String("poolLpPublicKey", swap.PoolLpPublicKey))
		}
	}
	...
	if tokenMetadata != nil && tokenMetadata.Ticker != "" {
		...
		holdingAmount, holdingValue := system_works.GetWalletTokenHolding(swap.SwapperPublicKey, swap.PoolLpPublicKey, swap, tokenTicker)
...
// newSwaps - swaps API)
func findNewSwapsBig(oldSwaps, newSwaps []system_works.Swap) []system_works.Swap {
	if len(oldSwaps) == 0 {
		return newSwaps
	}
	...
	var newSwapsList []system_works.Swap
...
func RunBigSalesBuysMonitor(bot *tgbotapi.BotAPI, client *system_works.Client, chatID string, minBTCAmount float64, filteredBot *tgbotapi.BotAPI, filteredChatID string, filteredTokensList []string, filteredMinBTCAmount float64) {
	system_works.LogInfo("Starting Big Sales/Buys Monitor...")
	...
	for {
		select {
		case <-reloadTokensTicker.C:
			if reloadTokensTicker != nil && filteredChatID != "" {
				newTokensList, err := system_works.LoadFilteredTokens()
				if err != nil {
					system_works.LogWarn("Failed to reload filtered tokens, using cached list", zap.Error(err))
				} else {
					filteredTokensList = newTokensList
					system_works.LogDebug("Reloaded filtered tokens from file", zap.Int("count", len(filteredTokensList)))
				}
			}
		...
		case <-ticker.C:
			...
			limit := 100
			swapsResp, err := client.GetSwaps(ctx, system_works.GetSwapsOptions{
				Limit: &limit, // 100 swaps
			})
			if err != nil {
				system_works.LogError("Failed to get swaps", zap.Error(err))
				continue
			}

			// Load from file for
			oldSwapsResp, _ := system_works.LoadSwapsResponse("big_sales_module/100_swaps.json")
			var oldSwaps []system_works.Swap
			if oldSwapsResp != nil {
				oldSwaps = oldSwapsResp.Swaps
			}

			// Save in file big_sales_module/100_swaps.json
			err = system_works.SaveSwapsResponse("big_sales_module/100_swaps.json", swapsResp)
			if err != nil {
				system_works.LogWarn("Failed to save swaps response", zap.Error(err))
			} else {
				system_works.LogInfo("Saved swaps to big_sales_module/100_swaps.json", zap.Int("count", len(swapsResp.Swaps)), zap.Int("totalAvailable", swapsResp.TotalCount))
			}

			newSwaps := findNewSwapsBig(oldSwaps, swapsResp.Swaps)

			if len(newSwaps) > 0 {
				system_works.LogInfo("Found new swaps", zap.Int("count", len(newSwaps)))
				...
				for _, swap := range newSwaps {
					...
					if bot != nil && chatID != "" {
						if shouldSendSwap(swap, minBTCAmount) {
							...
							_, err := bot.Send(msg)
							if err != nil {
								system_works.LogError("Failed to send message", zap.Error(err))
							} else {
								system_works.LogInfo("Sent swap notification", zap.String("swapID", swap.ID))
					...
					if filteredBot != nil && filteredChatID != "" && len(filteredTokensList) > 0 {
						...
						if isFilteredToken(swap.PoolLpPublicKey, filteredTokensList) {
							if shouldSendSwap(swap, filteredMinBTCAmount) {
								message, tradeLink := formatSwapMessageForTelegram(client, swap)
								...
								if err != nil {
									system_works.LogError("Failed to send filtered token message", zap.Error(err), zap.String("chatID", filteredChatID), zap.Bool("isSOON", isSOON))
								} else {
									system_works.LogInfo("Sent filtered token notification", zap.String("swapID", swap.ID), zap.String("poolLpPublicKey", swap.PoolLpPublicKey), zap.Bool("isSOON", isSOON), zap.String("swapType", string(swapType)))
									// Save address in saved_holders.json
									saveHolderFromSwap(swap)
								}
							}
...
func checkAndRefreshToken(client *system_works.Client) {
	dataDir := "data_in"
	...
	if publicKey == "" {
		system_works.LogWarn("Cannot refresh token: public key not found")
		return
	}

	system_works.LogInfo("Token expired or invalid, refreshing...")
	ctx := context.Background()

	// Get challenge
	_, err = client.GetChallengeAndSave(ctx, dataDir, publicKey)
	if err != nil {
		system_works.LogError("Failed to get challenge for token refresh", zap.Error(err))
		return
	}
	...
	if err != nil {
		system_works.LogError("Failed to sign challenge for token refresh",
			zap.Error(err),
	...
	if err != nil || sigFile.Signature == "" {
		system_works.LogError("Signature file not found after signing")
		return
	}

	_, err = client.VerifySignatureAndSave(ctx, dataDir, sigFile.PublicKey, sigFile.Signature)
	if err != nil {
		system_works.LogError("Failed to verify signature for token refresh", zap.Error(err))
		return
	}
...
// minBTCAmount - amount in BTC for
func RunFilteredTokensMonitor(bot *tgbotapi.BotAPI, client *system_works.Client, chatID string, filteredTokensList []string, minBTCAmount float64) {
	system_works.LogInfo("Starting Filtered Tokens Monitor...", zap.Int("filteredTokensCount", len(filteredTokensList)))
	...
	for {
		...
		case <-ticker.C:
			...
			if len(newSwaps) > 0 {
				system_works.LogInfo("Found new swaps for filtered monitor", zap.Int("count", len(newSwaps)))
				...
				if bot != nil && chatID != "" && len(filteredTokensList) > 0 {
					for _, swap := range newSwaps {
						...
						_, err := bot.Send(msg)
						if err != nil {
							system_works.LogError("Failed to send filtered token message", zap.Error(err), zap.String("chatID", chatID))
						} else {
							system_works.LogInfo("Sent filtered token notification", zap.String("swapID", swap.ID), zap.String("poolLpPublicKey", swap.PoolLpPublicKey))
							// Save address in saved_holders.json
							saveHolderFromSwap(swap)
						}
					}
