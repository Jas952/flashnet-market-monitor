...
func GenerateVolumeChart() (string, error) {
	...
	for _, logoPath := range logoPaths {
		if _, err := os.Stat(logoPath); err == nil {
			...
			if err == nil {
				logoImg = img
				logoLoaded = true
				LogInfo("Loaded spark.png logo", zap.String("path", logoPath))
				break
			}
		}
	}
	...
	if logoLoaded {
		...
		LogWarn("Failed to load spark.png logo - file not found in any expected location", zap.Strings("tried_paths", logoPaths))
	}
	...
	for _, fontPath := range fontPaths {
		...
		if fileInfo, err := os.Stat(expandedPath); err == nil {
			if err := dc.LoadFontFace(expandedPath, fontSize); err == nil {
				...
				LogInfo("Successfully loaded Inter font",
					zap.String("path", expandedPath),
					zap.Int64("size", fileInfo.Size()))
				break
			} else {
				LogWarn("Font file exists but failed to load",
					zap.String("path", expandedPath),
					zap.Error(err))
			}
		}
	...
	if !fontLoaded {
		// If use
		LogWarn("Failed to load Inter font from any path, using default system font",
	...
	dailyVolumeValue := fmt.Sprintf("$%s", FormatUSDValue(currentVolume24H))
	...
	avgVolumeValue := fmt.Sprintf("$%s", FormatUSDValue(avgDailyVolume))
	dc.SetColor(color.White)
	...
	if fileInfo.Size() == 0 {
		os.Remove(filename)
		LogError("Chart file is empty after rendering", zap.String("filename", filename))
		return "", fmt.Errorf("chart file is empty after rendering")
	}

	LogInfo("Volume chart generated successfully",
