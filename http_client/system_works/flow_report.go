...
func GenerateFlowReport(ticker string, dateStr string) (string, error) {
	...
	poolLpPublicKey, err := GetPoolLpPublicKeyForTicker(ticker)
	if err != nil {
	...
	if poolStats.BuyVolume != "" {
		if _, err := fmt.Sscanf(poolStats.BuyVolume, "%f", &buyVolume); err != nil {
			LogWarn("Failed to parse buyVolume", zap.String("buyVolume", poolStats.BuyVolume), zap.Error(err))
			buyVolume = 0
		}
	}
	if poolStats.SellVolume != "" {
		if _, err := fmt.Sscanf(poolStats.SellVolume, "%f", &sellVolume); err != nil {
			LogWarn("Failed to parse sellVolume", zap.String("sellVolume", poolStats.SellVolume), zap.Error(err))
			sellVolume = 0
		}
	}
